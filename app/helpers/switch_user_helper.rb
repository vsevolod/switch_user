module SwitchUserHelper
    ...
    SwitchUser.available_users.each do |scope, user_proc|
      ...
      user_proc.call.each do |user|
        if user == current_user
          selected_user = switch_tag_value(user, id_name, scope)
        end
        options << SelectOption.new(switch_tag_label(user, name), switch_tag_value(user, id_name, scope))
      end
    end
  ...
  def switch_tag_value(user, id_name, scope)
    identifier = user.send(id_name)

    "#{scope}_#{identifier}"
  end

  def switch_tag_label(user, name)
